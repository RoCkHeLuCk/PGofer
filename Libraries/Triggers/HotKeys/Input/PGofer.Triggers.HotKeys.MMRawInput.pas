...
const
  ...
  // the return valus is the character length, not the byte size
...
function GetRawInputDeviceInfoA( hDevice: THANDLE; uiCommand: UINT;
...
{$IFDEF UNICODE}
function GetRawInputDeviceInfo( hDevice: THANDLE; uiCommand: UINT;
  pData: Pointer; var pcbSize: UINT ): UINT; stdcall;
...
const
  ...
  // effective when mouse nolegacy is specified, otherwise it would be an error
...
function DefRawInputProc( HWND: HWND; Msg: UINT; wParam: wParam;
...
function RAWINPUT_ALIGN( x: Pointer ): Pointer;
begin
  Result := Pointer( ( Integer( x ) + SizeOf( DWORD ) - 1 ) and
    not( SizeOf( DWORD ) - 1 ) );
end;
...
begin
  GetProcedureAddress( _GetRawInputDeviceInfoA, user32,
...
begin
  GetProcedureAddress( _GetRawInputDeviceInfoW, user32,
...
begin
  GetProcedureAddress( _GetRawInputDeviceInfo, user32,
    'GetRawInputDeviceInfoW' );
  asm
    mov esp, ebp
    pop ebp
...
begin
  GetProcedureAddress( _RegisterRawInputDevices, user32,
...
begin
  GetProcedureAddress( _GetRegisteredRawInputDevices, user32,
...
{$ELSE}
function GetRegisteredRawInputDevices;
  external user32 name 'GetRegisteredRawInputDevices';
{$ENDIF DYNAMIC_LINK}
{$IFDEF DYNAMIC_LINK}
...
begin
  GetProcedureAddress( _GetRawInputDeviceList, user32,
