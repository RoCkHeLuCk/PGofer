...
begin
  ...
  // Envia todas as 256 ordens de "soltar" de uma s� vez para o Windows
...
begin
  ...
  begin
    ...
    begin
      // Verifica se o pr�ximo caractere completa o par
    ...
    // Usamos C.IsControl ou Ord(C) para l�gica personalizada
    if (CharInSet(C, [#9, #10, #13])) or (W >= 32) then
    begin
      if (W = $0B) or (W = $0C) then Continue;
      // Bloqueio de caracteres invis�veis/fantasmas espec�ficos
      if not (
