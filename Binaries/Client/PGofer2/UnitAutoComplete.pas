...
begin
    ...
    begin
        ...
        Ini.ReadSections(Se��o);
        //Varre lista de Se�oes
        for c:=0 to Se��o.Count-1 do
        begin
            if CompareText(Se��o[c],'Config') <> 0 then
            begin
                //adiciona link
                e := Length( AtalhoGlobal );
                SetLength(AtalhoGlobal, e+1);
                AtalhoGlobal[e].Nome := Se��o[c];
                AtalhoGlobal[e].Arquivo := Ini.ReadString(Se��o[c],'Arquivo','');
                AtalhoGlobal[e].Parametro := Ini.ReadString(Se��o[c],'Parametro','');
                AtalhoGlobal[e].Diretorio := Ini.ReadString(Se��o[c],'Diretorio','');
                AtalhoGlobal[e].Icone := Ini.ReadString(Se��o[c],'Icone','');
                AtalhoGlobal[e].ShowControl := Ini.ReadInteger(Se��o[c],'Abrir',1);
            end;//if config
        end;// for
    //limpar
        Ini.Free;
...
//----------------------------------------------------------------------------//
procedure TFrmAutoCompletes.CarregarFun��o();
var
    texto : string;
    c, d, e, f : integer;
begin
...
begin
    //arruma a baga�a para n�o dar um bug sinistro.
    Width := Width -1;
    Update;
    Width := Width +1;
end;
...
var
    v1,v2 : integer;
begin
    //ajusta a ordem dos comando por valor de execu��o.
